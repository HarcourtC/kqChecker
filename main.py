...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="main.py", description="Launcher for kq.scheduler"
    )
    parser.add_argument(
        "--once", action="store_true", help="Run one immediate pass and exit"
    )
    parser.add_argument(
        "--test",
        action="store_true",
        help="Invoke the scheduler in test mode for the next event",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="When used with --once or --test, skip network calls",
    )
    parser.add_argument(
        "--schedule",
        "-s",
        help="Path to schedule JSON file (defaults to weekly.json in project root)",
    )
