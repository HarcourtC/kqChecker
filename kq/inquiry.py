...
def post_attendance_query(
    ...
    if not isinstance(event_time, datetime):
        logging.warning(
            "post_attendance_query: event_time should be datetime, got %s",
            type(event_time),
        )
    ...
    try:
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if courses:
                    ...
                    if isinstance(courses, list):
                        for c in courses:
                            if isinstance(c, dict):
                                ...
                                scheduled_entries.append(
                                    {"course": c, "room": None, "raw": None}
                                )
                    elif isinstance(courses, str):
                        course_names = [courses]
                        scheduled_entries = [
                            {"course": courses, "room": None, "raw": None}
                        ]
                    else:
                        # fallback
                        try:
                    ...
                    if not matched:
                        logging.info(
                            "no direct name-based attendance records found for courses=%s on %s",
                            courses,
                            date_str,
                        )
                        ...
                        try:
                            ...
                            weekly_single = {
                                event_time.strftime("%Y-%m-%d %H:%M:%S"): course_names
                            }
                            # use -20/+5 minute window as tested previously
                            time_matches = match_records_by_time(
                                resp_json,
                                weekly_single,
                            ...
                            total_candidates = sum(
                                len(v) for v in time_matches.values()
                            )
                            if total_candidates:
                                logging.info(
                                    "time-based matching found %d candidate attendance record(s) for courses=%s on %s",
                                    total_candidates,
                                    course_names,
                                    date_str,
                                )
                                ...
                                for k, recs in time_matches.items():
                                    ...
                                    for idx, course_name in enumerate(
                                        scheduled_course_names
                                    ):
                                        scheduled_room = None
                                        try:
                                            scheduled_room = (
                                                scheduled_entries[idx].get("room")
                                                if idx < len(scheduled_entries)
                                                else None
                                            )
                                        except Exception:
                                            scheduled_room = None
                                        for r in recs:
                                            ...
                                            try:
                                                ...
                                                snippet = {
                                                    "operdate": r.get("operdate")
                                                    or r.get("watertime")
                                                    or r.get("intime"),
                                                    "teacher": r.get("teachNameList"),
                                                    "subject": (
                                                        (
                                                            r.get("subjectBean") or {}
                                                        ).get("sName")
                                                        if isinstance(
                                                            r.get("subjectBean"), dict
                                                        )
                                                        else None
                                                    ),
                                                    "rec_room": rec_room,
                                                ...
                                                logging.debug(
                                                    "time-match candidate for %s: %s",
                                                    k,
                                                    snippet,
                                                )
                                            except Exception:
                                                logging.debug(
                                                    "error while logging time-match candidate",
                                                    exc_info=True,
                                                )
                                            # compare normalized room strings (if both present)
                                            if scheduled_room and rec_room:
                                                if _norm(scheduled_room) in _norm(
                                                    rec_room
                                                ) or _norm(rec_room) in _norm(
                                                    scheduled_room
                                                ):
                                                    logging.info(
                                                        "time+room match accepted for course=%s on %s (rec_room=%s sched_room=%s)",
                                                        course_name,
                                                        date_str,
                                                        rec_room,
                                                        scheduled_room,
                                ...
                                if matched_by_room:
                                    ...
                                    logging.info(
                                        "time-based candidates found but none matched by room for courses=%s on %s",
                                        course_names,
                                        date_str,
                                    )
                                    ...
                                    try:
                                        ...
                                        for k2, recs2 in time_matches.items():
                                            for r2 in recs2:
                                                cand = {
                                                    ...
                                                    "rbh": r2.get("rbh")
                                                    or r2.get("bh"),
                                                    "cardId": r2.get("cardId")
                                                    or r2.get("cardid"),
                                                    "watertime": r2.get("watertime"),
                                                    "intime": r2.get("intime"),
                                                    "subject": (
                                                        (
                                                            r2.get("subjectBean") or {}
                                                        ).get("sName")
                                                        if isinstance(
                                                            r2.get("subjectBean"), dict
                                                        )
                                                        else None
                                                    ),
                                                    "rec_room": _extract_room_from_record(
                                                        r2
                                                    ),
                                        ...
                                        logging.info(
                                            "time-match candidates detail: %s",
                                            json.dumps(cand_list, ensure_ascii=False),
                                        )
                                    except Exception:
                                        logging.debug(
                                            "failed to serialize time-match candidates for logging",
                                            exc_info=True,
                                        )
                            else:
                                logging.info(
                                    "no time-based candidates found either for courses=%s on %s",
                                    course_names,
                                    date_str,
                                )
                        except Exception:
                        ...
                        try:
                            ...
                            try:
                                ...
                                if (
                                    ...
                                    fname = (
                                        f"missing_{date_str}_{safe_courses}_{ts}.json"
                                    )
                                    import re

                                    fname = re.sub(
                                        r"[^0-9A-Za-z_\-\.\u4e00-\u9fff]", "", fname
                                    )
                                    outp = dump_dir / fname
                                    try:
                                        ...
                                        logging.info(
                                            "saved missing POST response to %s", outp
                                        )
                                    except Exception:
                                        logging.exception(
                                            "failed to write missing POST response to %s",
                                            outp,
                                        )
                            except Exception:
                                logging.debug(
                                    "error while attempting to save missing response for debugging",
                                    exc_info=True,
                                )
                            ...
                            send_miss_email_async(
                                cfg, subject=None, body=None, context=context
                            )
                        except Exception:
                    ...
                    try:
                        ...
                        if notifs.get("on_match"):
                            ...
                            try:
                                send_miss_email_async(
                                    cfg, subject=subj, body=body, context=context
                                )
                                logging.info(
                                    "match notification scheduled (on_match enabled)"
                                )
                            except Exception:
                                logging.exception(
                                    "failed to schedule match notification"
                                )
                    except Exception:
                        logging.debug(
                            "error while attempting to send match notification",
                            exc_info=True,
                        )
                    for rec in cleaned:
                ...
                # If a deliberate API400Error was raised, re-raise so callers can handle it
                if isinstance(e, API400Error):
                    raise
                last_exc = e
                logging.warning(
        ...
        # propagate API400Error to caller instead of swallowing it here
        if isinstance(exc, API400Error):
            raise
...
def extract_course_records(
    response_json: Dict[str, Any], course_names: List[str]
...
def clean_records(records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in records:
        try:
            ...
            room = (
                item.get("roomBean")
                or (item.get("classWaterBean") or {}).get("roomBean")
                or {}
            )
