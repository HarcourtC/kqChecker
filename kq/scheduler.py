...
def setup_logging(log_file: Optional[Path] = None) -> None:
    ...
    try:
        ...
        root_link.write_text(
            f"Current log file: logs/{log_path.name}\n", encoding="utf-8"
        )
    except Exception:
...
def check_attendance(event_time, entries, dry_run: bool = False) -> None:
    ...
    logging.info(
        "checking attendance for %s at %s", course_names, event_time.isoformat()
    )
    try:
        if dry_run:
        ...
        if not (0 < minutes_before <= 5):
            logging.info(
                "not the notify moment (%.1f minutes before); skipping for %s",
                minutes_before,
                course_names,
            )
            return
    ...
    while True:
        try:
            ...
            for start_dt, courses in events:
                ...
                names = [
                    str(c.get("course") if isinstance(c, dict) else c or "")
                    for c in courses
                ]
                ...
                if check_time <= now < start_dt:
                    logging.info(
                        "triggering attendance check for %s (starts at %s)",
                        courses,
                        start_dt,
                    )
                    try:
                        ...
                        logging.exception(
                            "error while checking attendance for %s", start_dt
                        )
                    processed.add(key)
            ...
            try:
                ...
                if today.weekday() == 6 and last_weekly_refresh != today:
                    logging.info(
                        "weekly refresh: detected Sunday, regenerating weekly.json and saving raw response"
                    )
                    try:
                        ...
                        if gw.exists():
                            ...
                            logging.warning(
                                "get_weekly_json.py not found at %s, skipping regeneration",
                                gw,
                            )
...
def main() -> None:
    ...
    try:
        ...
        logging.debug(
            "error while attempting to send startup notification", exc_info=True
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="kq.scheduler: run attendance scheduler or one-shot/test runs"
    )
    parser.add_argument(
        "--once",
        action="store_true",
        help="Run one immediate pass: process events due within the next 5 minutes and exit",
    )
    parser.add_argument(
        "--test",
        action="store_true",
        help="Test run: invoke check_attendance for the next scheduled event and exit",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="When used with --once or --test, skip network calls and only exercise the flow",
    )
    parser.add_argument(
        "--schedule",
        "-s",
        help="Path to schedule JSON file (defaults to project's weekly.json)",
    )
    ...
    if args.once:
        ...
        for start_dt, courses in events:
            ...
            if check_time <= now < start_dt:
                logging.info(
                    "one-shot: triggering attendance check for %s (starts at %s)",
                    courses,
                    start_dt,
                )
        ...
        if next_ev:
            ...
            logging.info(
                "test run: invoking check_attendance for next event %s (starts at %s)",
                courses,
                start_dt,
            )
            check_attendance(start_dt, courses, dry_run=args.dry_run)
